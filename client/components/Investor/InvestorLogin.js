...
const InvestorLogin = ({ navigation }) => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  const handleLogin = async () => {
    try {
      const response = await axios.post(`${BASE_URL}/api/InvLogin`, {
        email,
        password,
      });
      ...
      navigation.navigate('InvestorHome'); // Navigate to the dashboard or any other screen
    } catch (error) {
...
<TouchableOpacity onPress={() => navigation.navigate('InvestorRegister')}>
